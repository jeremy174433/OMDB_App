...
/*
  Generated class for the BddProvider provider.
...
export class BddProvider {
  ...
  constructor(public http: HttpClient, private storage: Storage) {
    console.log('Hello BddProvider Provider');
  }
  ...
  verification_favoris() {
     ...
     /*console.log( "RetourPromise")
      console.log( RetourPromise)
      if (RetourPromise == false )
      {
        return false }else
          {
            for ( var i = 0 ; i < this.TableauFavoris.length ; i++ )
            {
              if ( this.TableauFavoris[i].id == id   )
              {
                return true
              }
    }
